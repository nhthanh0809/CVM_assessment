...
class CephaLandmark_v3(nn.Module):
    def __init__(self, points_num=17):
        super(CephaLandmark_v3, self).__init__()

        # self.features = models.densenet121(pretrained=True).features

        self.mobile_features = models.mobilenet_v2(pretrained=True).features
    ...
    def forward(self, x):
        ...
        # for param in self.mobile_features.parameters():
        #     print(param.requires_grad)

        x = self.mobile_features[:1](x)
        # x = self.features[0](x)
        w1_f1 = x
        # print("features[0]: ", w1_f1.shape)
        x = self.mobile_features[1:4](x)
        # for i in range(1, 5):
        #     x = self.features[i](x)
        w1_f2 = x
        # print("features[4]: ", w1_f2.shape)

        x = self.mobile_features[4:7](x)
        # for i in range(5, 7):
        #     x = self.features[i](x)
        w1_f3 = x
        # print("features[6]: ", w1_f3.shape)

        x = self.mobile_features[7:14](x)
        # for i in range(7, 9):
        #     x = self.features[i](x)
        w1_f4 = x
        ...
        # for i in range(9, 12):
        #     x = self.features[i](x)
        x = self.mobile_features[14:18](x)
...
#     model = CephaLandmark_v3(points_num=17)
